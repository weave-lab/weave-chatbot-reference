...
def create_genai_client(client_type: str) -> genai.Client:
    """Create a GenAI client based on the specified client type."""
    if client_type == "vertex_ai":
        return genai.Client(vertexai=True, project=PROJECT_ID, location=LOCATION)
    elif client_type == "gemini_api":
        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key:
            raise ValueError(
                "GEMINI_API_KEY environment variable must be set for gemini_api client"
            )
        return genai.Client(api_key=api_key)
    else:
        raise ValueError(
            f"Unsupported client type: {client_type}. Use 'vertex_ai' or 'gemini_api'"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "-c",
        default="vertex_ai",
        choices=["vertex_ai", "gemini_api"],
        help="Client type to use: vertex_ai (default) or gemini_api",
    )
    args = parser.parse_args()

    # Initialize GenAI Client based on the specified client type
    genai_client = create_genai_client(args.client)
